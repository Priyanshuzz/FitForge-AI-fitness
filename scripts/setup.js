...
async function main() {
  ...
  log('You can skip any step by pressing Enter (will use default/placeholder)\n', 'yellow');
  ...
  config.NEXT_PUBLIC_SUPABASE_URL = await question('Supabase Project URL (or press Enter to skip): ');
  if (!config.NEXT_PUBLIC_SUPABASE_URL) {
    config.NEXT_PUBLIC_SUPABASE_URL = 'your_supabase_project_url';
  }
  
  config.NEXT_PUBLIC_SUPABASE_ANON_KEY = await question('Supabase Anon Key (or press Enter to skip): ');
  ...
  config.OPENAI_API_KEY = await question('OpenAI API Key (or press Enter to skip): ');
  ...
  const appUrl = await question(`App URL (default: ${config.NEXT_PUBLIC_APP_URL}): `);
  if (appUrl) {
...
`;
  ...
  try {
    ...
    const isSupabaseConfigured = config.NEXT_PUBLIC_SUPABASE_URL !== 'your_supabase_project_url' && 
                                config.NEXT_PUBLIC_SUPABASE_ANON_KEY !== 'your_supabase_anon_key';
    const isOpenAIConfigured = config.OPENAI_API_KEY !== 'your_openai_api_key';
    
    log('\n📊 CONFIGURATION STATUS:', 'bold');
    log(`Database & Auth: ${isSupabaseConfigured ? '✅ Configured' : '❌ Not configured'}`, 
        isSupabaseConfigured ? 'green' : 'red');
    log(`AI Features: ${isOpenAIConfigured ? '✅ Configured' : '❌ Not configured'}`, 
        isOpenAIConfigured ? 'green' : 'red');
    
    if (!isSupabaseConfigured || !isOpenAIConfigured) {
      log('\n⚠️  Some features are not configured:', 'yellow');
      if (!isSupabaseConfigured) {
        log('   • Set up Supabase for authentication and data storage', 'yellow');
      }
      if (!isOpenAIConfigured) {
        log('   • Set up OpenAI for AI-powered features', 'yellow');
      }
      log('\nYou can edit .env.local manually or run this script again.', 'yellow');
