...
export function WorkoutProgressChart({ userId, height = 300, detailed = false }: WorkoutProgressChartProps) {
        ...
        <div className="bg-background border border-border rounded-lg p-3 shadow-md">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} style={{ color: entry.color }}>
              ...
              {entry.dataKey === 'totalDuration' && `Duration: ${entry.value} min`}
      ...
      <Tabs value={timeRange} onValueChange={(value) => setTimeRange(value as 'week' | 'month')}>
        ...
        <TabsContent value={timeRange} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h4 className="text-lg font-semibold mb-4">Total Duration (min)</h4>
          ...
          <div>
            <h4 className="text-lg font-semibold mb-4">Workout Type Distribution</h4>
              ...
              <div className="flex-1 space-y-2">
                {workoutTypeDistribution.map((entry, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                    ...
                    <span className="text-muted-foreground">{entry.value} sessions</span>
                  </div>
                ))}
              </div>
            </div>
