...
export function PhysicalProfileStep({ data, errors, onChange }: PhysicalProfileStepProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div>
          <Label className="text-lg font-semibold">Current Activity Level *</Label>
        ...
        <RadioGroup
          ...
          onValueChange={(value) => handleInputChange('activity_level', value as ActivityLevel)}
        >
      ...
      <div className="space-y-4">
        <div>
          <Label className="text-lg font-semibold">Fitness Experience Level *</Label>
          <p className="text-sm text-muted-foreground mt-1">
        ...
        <RadioGroup
          ...
          onValueChange={(value) => handleInputChange('fitness_level', value as FitnessLevel)}
        >
          <div className="grid gap-3">
      ...
      <div className="space-y-4">
        ...
        <div className="px-2">
          <Slider
            ...
            onValueChange={(value) => handleInputChange('days_per_week', value[0])}
            max={7}
            min={1}
            step={1}
            className="w-full"
        ...
        <div className="bg-muted/50 p-3 rounded-lg">
          <p className="text-sm">
            {data.days_per_week === 1 && "Perfect for building a habit! We'll make every session count."}
            {data.days_per_week === 2 && "Great start! Two quality sessions can make a real difference."}
            {data.days_per_week === 3 && "Excellent balance! Three days allows for good progress and recovery."}
            {data.days_per_week === 4 && "Strong commitment! Four days will accelerate your progress."}
            {data.days_per_week === 5 && "High dedication! Five days will deliver significant results."}
            {data.days_per_week === 6 && "Very ambitious! Make sure to prioritize recovery and nutrition."}
            {data.days_per_week === 7 && "Maximum commitment! We'll include active recovery to prevent burnout."}
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="px-2">
          <Slider
            ...
            onValueChange={(value) => handleInputChange('session_minutes', value[0])}
            max={120}
            min={15}
            step={15}
            className="w-full"
        ...
        <div className="bg-muted/50 p-3 rounded-lg">
          <p className="text-sm">
            {data.session_minutes && data.session_minutes <= 30 && "Short and efficient! Perfect for busy schedules."}
            {data.session_minutes && data.session_minutes > 30 && data.session_minutes <= 60 && "Ideal duration for comprehensive workouts with good results."}
            {data.session_minutes && data.session_minutes > 60 && "Extended sessions allow for detailed warm-up, training, and recovery."}
          </p>
        </div>
      ...
      <div className="bg-primary/5 border border-primary/20 p-4 rounded-lg">
        ...
        <ul className="text-sm space-y-1">
          <li>• Be honest about your current activity level - this ensures safe progression</li>
          <li>• It's better to start conservative and build up gradually</li>
          <li>• Your fitness level helps us choose appropriate exercises and intensity</li>
