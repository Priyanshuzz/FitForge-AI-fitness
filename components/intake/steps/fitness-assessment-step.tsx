...
const TRAINING_STYLES = [
  {
    ...
    icon: Dumbbell,
    benefits: ['Increase bone density', 'Build functional strength', 'Improve metabolism']
  },
  {
    ...
    icon: Target,
    benefits: ['Increase muscle mass', 'Improve body composition', 'Enhanced physique']
  },
  {
    ...
    icon: Zap,
    benefits: ['Burn calories efficiently', 'Improve cardiovascular health', 'Time-efficient']
  },
  {
  ...
  },
  {
    ...
    icon: Activity,
    benefits: ['No equipment needed', 'Functional movements', 'Improved mobility']
  },
  {
  ...
  },
  {
    ...
    icon: Waves,
    benefits: ['Core strengthening', 'Improved posture', 'Mind-body connection']
  },
  {
    ...
    icon: Shuffle,
    benefits: ['Prevents boredom', 'Well-rounded fitness', 'Adaptable approach']
  }
];
...
export function FitnessAssessmentStep({ data, errors, onChange }: FitnessAssessmentStepProps) {
  const selectedTrainingStyles = data.training_styles || [];
  const selectedEquipment = data.equipment || [];

  const handleTrainingStyleChange = (style: TrainingStyle, checked: boolean) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div>
          <Label className="text-lg font-semibold">Training Style Preferences *</Label>
          ...
          {TRAINING_STYLES.map((style) => {
            ...
            return (
              <Card 
                ...
                onClick={() => handleTrainingStyleChange(style.value, !isSelected)}
              >
                  ...
                  <div className="flex items-start space-x-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => 
                        handleTrainingStyleChange(style.value, checked as boolean)
                      }
                      className="mt-1"
                    />
        ...
        {selectedTrainingStyles.length > 0 && (
          <div className="bg-primary/5 border border-primary/20 p-3 rounded-lg">
            <p className="text-sm font-medium">Selected: {selectedTrainingStyles.length} style(s)</p>
      ...
      <div className="space-y-4">
        ...
        {['Bodyweight', 'Free Weights', 'Accessories', 'Equipment', 'Machines', 'Cardio', 'Comprehensive'].map(category => {
          const categoryEquipment = EQUIPMENT_OPTIONS.filter(eq => eq.category === category);
                    ...
                    >
                      <CardContent className="p-3">
                        <div className="flex items-start space-x-3">
                          ...
                          />
                          <div className="flex-1">
                            <h5 className="font-medium text-sm">{equipment.label}</h5>
        ...
        {selectedEquipment.length > 0 && (
          <div className="bg-primary/5 border border-primary/20 p-3 rounded-lg">
            <p className="text-sm font-medium">Selected: {selectedEquipment.length} equipment type(s)</p>
            <div className="flex flex-wrap gap-1 mt-2">
      ...
      <div className="bg-muted/50 p-4 rounded-lg">
        ...
        <ul className="text-sm space-y-1">
          <li>• <strong>Multiple styles:</strong> Creates variety and prevents plateaus</li>
          <li>• <strong>Equipment flexibility:</strong> More options = more exercise variety</li>
          <li>• <strong>No equipment?</strong> Bodyweight training can be highly effective</li>
          <li>• <strong>Full gym access:</strong> Unlocks the most comprehensive program options</li>
        </ul>
      </div>
    </div>
  );
